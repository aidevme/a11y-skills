...
| TC-P4.2-05 | Write to `ControlManifest.Input.xml` with a violation | **Known gap:** the hook classifies this extension as a recognized UI surface but currently **allows** the write rather than denying it — no PCF rule pack exists to lint it against until Phase 5. Revisit this scenario once `packages/rules-pcf` ships. |
